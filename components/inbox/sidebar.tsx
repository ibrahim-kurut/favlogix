...
import { Users, UserX, ChevronDown, ChevronRight, MessageCircle, User, X } from "lucide-react"
...
const inboxItems = [
  { icon: User, label: "My Inbox", count: null },
  { icon: Users, label: "All", count: 28 },
  { icon: UserX, label: "Unassigned", count: 5 },
]

const teams = [
  { icon: MessageCircle, label: "Sales", count: 7 },
  { icon: MessageCircle, label: "Customer Support", count: 16 },
]

const users = [
  { name: "Sarah Williams", count: 2 },
  { name: "Michael Johnson", count: 11, active: true },
  { name: "Emily Davis", count: null },
  { name: "Christopher Miller", count: 4 },
  { name: "Amanda Garcia", count: 5 },
  { name: "Joshua Martinez", count: null },
  { name: "Ashley Taylor", count: 1 },
  { name: "Daniel Anderson", count: null },
  { name: "Jessica Thomas", count: 2 },
]

const channels = [
  { name: "Fit4Life", icon: "whatsapp", color: "bg-green-500" },
  { name: "Fit4Life", icon: "instagram", color: "bg-gradient-to-br from-purple-500 to-pink-500" },
]
...
export function Sidebar({ collapsed, onToggle, onClose }: SidebarProps) {
  ...
  return (
    <aside className="flex w-52 flex-col border-r border-border bg-card h-full">
      ...
      <nav className="flex-1 space-y-1 overflow-y-auto px-2">
        ...
        <div className="pt-4">
          ...
          >
            <span>Teams</span>
            {teamsOpen ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
          </button>
          {teamsOpen && (
            ...
            <div className="space-y-1">
              {users.map((user) => (
                ...
                >
                  ...
                  {user.count !== null && <span className="text-xs text-muted-foreground">{user.count}</span>}
                </button>
              ))}
            </div>
          )}
          ...
          {channelsOpen && (
            <div className="space-y-1">
              {channels.map((channel, index) => (
                ...
                >
                  <div className={cn("h-5 w-5 rounded-full flex items-center justify-center", channel.color)}>
